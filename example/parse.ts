...
async function parse() {
    ...
    console.log("Expiration Timestamp:", transaction.expiration_timestamp_secs.toString());
    ...
    if (signatureBytes.length >= 65) {
      ...
      console.log("Public Key (hex):", Array.from(publicKey).map(b => b.toString(16).padStart(2, '0')).join(''));
      console.log("Signature (hex):", Array.from(signature).map(b => b.toString(16).padStart(2, '0')).join(''));
    ...
    console.log("Transaction (hex):", Array.from(transactionBytes).map(b => b.toString(16).padStart(2, '0')).join(''));
